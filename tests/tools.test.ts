...
describe('ToolRegistry builtins hardening', () => {
  ...
  it('file_read returns content written by file_store for the same run', async () => {
    const registry = new ToolRegistry();
    const ctx = {
      runId: 'r-file-read-roundtrip',
      tokenOwner: 'o-fr',
      tryReserveArtifactsBytes: () => true,
      rollbackArtifactsBytes: () => {}
    };

    // Write a file
    const written = await registry.call('file_store', { path: 'output.json', content: '{"x":42}' }, ctx);
    expect(written.ok).toBe(true);

    // Read it back immediately
    const read = await registry.call('file_read', { path: 'output.json' }, ctx);
    expect(read.ok).toBe(true);
    const out = read.output as { path: string; content: string; bytes: number };
    expect(out.content).toBe('{"x":42}');
    expect(out.path).toBe('output.json');
    expect(out.bytes).toBe(8);
  });

  it('file_read blocks path traversal attacks', async () => {
    const registry = new ToolRegistry();
    const ctx = { runId: 'r-fr-sec', tokenOwner: 'o-sec' };

    const traversal = await registry.call('file_read', { path: '../other-run/secret.txt' }, ctx);
    expect(traversal.ok).toBe(false);
    expect((traversal.output as { error: string }).error).toBe('PATH_NOT_ALLOWED');

    const absolute = await registry.call('file_read', { path: '/etc/passwd' }, ctx);
    expect(absolute.ok).toBe(false);
    expect((absolute.output as { error: string }).error).toBe('PATH_NOT_ALLOWED');
  });

  it('file_read returns FILE_NOT_FOUND for nonexistent paths', async () => {
    const registry = new ToolRegistry();
    const result = await registry.call('file_read', { path: 'does-not-exist.txt' }, { runId: 'r-fr-missing', tokenOwner: 'o-m' });
    expect(result.ok).toBe(false);
    expect((result.output as { error: string }).error).toBe('FILE_NOT_FOUND');
  });
});
