...
export class ExecutionEngine {
  ...
  private async executeTask(task: TaskSpec, run: Run, opts: RunOptions, signal: AbortSignal): Promise<void> {
    ...
    while (attempts < 3) {
      ...
      try {
        ...
        // Verifier must explicitly return { verified: true }. Any other value — false, null,
        // undefined, or unexpected structure — is treated as a verification failure so that
        // silent pass-throughs (model returning wrong format) don't hide real issues.
        if (task.agent === 'verifier' && attemptResult.payload?.verified !== true) {
          const issues = Array.isArray(attemptResult.payload?.issues) ? attemptResult.payload.issues : [];
          const msg = issues.length > 0
            ? `Verifier rejected results: ${(issues as unknown[]).join('; ')}`
            : 'Verifier did not confirm success (verified !== true)';
          throw this.err('VERIFIER_FAILED', msg, true, 'upgrade', task.name);
        }
        ...
        if (typed.retryable && attempts < 3) {
          // Rate limit errors need much longer back-off than transient network errors.
          const delayMs = typed.code === 'RATE_LIMIT'
            ? Math.min(10_000, 1_000 * attempts)   // 1 s, 2 s, capped at 10 s
            : 20 * Math.pow(2, attempts - 1);        // 20 ms, 40 ms, 80 ms
  ...
  private buildFinalOutput(run: Run): unknown {
    ...
    // For multi-mode: executor payloads are the primary result.
    // Triage / planner / verifier are pipeline intermediaries — don't surface them as final output.
    const pipelineRoles = new Set(['triage', 'planner', 'verifier']);
    const executorEntries = Object.entries(run.results_by_task)
      .filter(([name]) => !pipelineRoles.has(name));

    const verifierPayload = run.results_by_task.verifier?.payload as
      | { verified?: boolean; issues?: unknown[] }
      | undefined;

    if (executorEntries.length === 0) {
      return { summary: 'No executor results', verified: verifierPayload?.verified ?? null };
    }

    if (executorEntries.length === 1) {
      // Single executor path: return its payload directly (clean, contract-compatible)
      return executorEntries[0][1].payload;
    }

    // Multiple executors: return a structured aggregate with verification status
    return {
      summary: 'Multi-agent run completed',
      verified: verifierPayload?.verified ?? null,
      issues: verifierPayload?.issues ?? [],
      results: Object.fromEntries(executorEntries.map(([k, v]) => [k, v.payload]))
    };
  }
