...
export const ModelPricingEntrySchema = z.object({
  /** OpenRouter canonical model ID, e.g. "anthropic/claude-3-5-sonnet-20241022". */
  id: z.string().min(1),
  /** Human-readable model name from OpenRouter. */
  name: z.string(),
  /** Tier this model was classified into based on its prompt price. */
  tier: z.enum(['cheap', 'standard', 'premium']),
  /** Prompt token price in USD per 1 million tokens. */
  prompt_per_1m: z.number().nonnegative(),
  /** Completion token price in USD per 1 million tokens. */
  completion_per_1m: z.number().nonnegative(),
  /** True when the model comes directly from its primary provider on OpenRouter. */
  is_top_provider: z.boolean()
});
